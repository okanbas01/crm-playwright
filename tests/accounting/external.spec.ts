...
import ExternalPage from '../../pages/accounting/external';
...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(`${baseURL}/accounting/accounts/externals`);
  await page.waitForTimeout(2000);
});

test('Accounting External Accounts Create Cash', async ({ page }) => {
  const external = new ExternalPage(page);
  await external.createNewAccountGroupCash();
});
test('Accounting External Accounts Create Bank', async ({ page }) => {
  const external = new ExternalPage(page);
  await external.createNewAccountGroupBank();
});
