...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(`${baseURL}/accounting/accounts/customer-accounts`);
  await page.waitForTimeout(2000);
});

test('Accounting Customer Accounts Create Cash', async ({ page }) => {
  const customer = new CustomerPage(page);
  await customer.createNewCustomerAccountGroupCash(values.customerNameText);
});
test('Accounting Customer Accounts Create Bank', async ({ page }) => {
  const customer = new CustomerPage(page);
  await customer.createNewCustomerAccountGroupBank();
});
