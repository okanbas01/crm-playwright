...
// });
test('New Transform', async ({ page }) => {
  const loan = new CustomerCurrentsPage(page);
...
test('Customer Current Currency Change', async ({ page }) => {
  const exchange = new CustomerCurrentsPage(page);
  await exchange.customerCurrentCurrencyChange();
});
